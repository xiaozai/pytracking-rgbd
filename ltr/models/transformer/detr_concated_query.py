...
from .resnet_backbone import get_resnet_backbone, get_position_encoding
from .transformer import build_transformer_query, build_transformer_concated_query
from ltr.external.PreciseRoIPooling.pytorch.prroi_pool import PrRoIPool2D

class DETR_CONCATE_QUERY(nn.Module):
    """
        Song : This is the DETR module that performs object tracking
    """
    def __init__(self, backbone, pos_embed, transformer, output_layers=['layer4'], num_channels=2048, query_encoder_size=9, roi_windowsize=3, prroipool_scalefactor=1/32):
        """ Initializes the model.
        Parameters:
            backbone    : torch module of the backbone to be used. See backbone.py
            transformer : torch module of the transformer architecture. See transformer.py
            pos_emebd   : the learned positional embedding or the sin-cos positional embedding
        """
        super().__init__()
        ...
        self.query_proj_decoder = QueryProj_Decoder(num_channels, hidden_dim, roi_windowsize)
...
def build_tracker(backbone_name='resnet50',
    ...
    learned_pos_embed = get_position_encoding(hidden_dim=hidden_dim,
                                              position_embedding=position_embedding)

    transformer = build_transformer_concated_query(hidden_dim=hidden_dim,
                                                   dropout=dropout,
                                                   nheads=nheads,
    ...
    model = DETR_CONCATE_QUERY(backbone, learned_pos_embed, transformer,
                               num_channels=num_channels,
                               output_layers=output_layers,
                               query_encoder_size=query_encoder_size,
                               roi_windowsize=roi_windowsize,
                               prroipool_scalefactor=scale_factor)
