...
class DiMP(BaseTracker):
    ...
    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            ...
            self.params.net_rgb.initialize()
            self.params.net_d.initialize()
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat_rgb, backbone_feat_d, sample_coords_rgb, sample_coords_d, im_patches, dp_patches = self.extract_backbone_features(im, dp, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)
        # Extract classification features
        test_x_rgb, test_x_d = self.get_classification_features(backbone_feat_rgb, backbone_feat_d)

        # Location of sample
        sample_pos_rgb, sample_pos_d, sample_scales_rgb, sample_scales_d = self.get_sample_location(sample_coords_rgb, sample_coords_d)

        # Compute classification scores
        scores_raw_rgb, scores_raw_d = self.classify_target(test_x_rgb, test_x_d)
        print(scores_raw_rgb.shape, scores_raw_d.shape)

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw_rgb, sample_pos_rgb, sample_scales_rgb)
        new_pos = sample_pos_rgb[scale_ind,:] + translation_vec
        ...
        if flag != 'not_found':
            if self.params.get('use_iou_net', True):
                ...
                self.refine_target_box(backbone_feat_rgb, backbone_feat_d, sample_pos_rgb[scale_ind,:], sample_scales_rgb[scale_ind], scale_ind, update_scale_flag)
            elif self.params.get('use_classifier', True):
                self.update_state(new_pos, sample_scales_rgb[scale_ind])
        ...
        if update_flag and self.params.get('update_classifier', False):
            # Get train sample
            train_x = test_x_rgb[scale_ind:scale_ind+1, ...]

            # Create target_box and label for spatial sample
            target_box = self.get_iounet_box(self.pos, self.target_sz, sample_pos_rgb[scale_ind,:], sample_scales_rgb[scale_ind])
        ...
        self.search_area_box = torch.cat((sample_coords_rgb[scale_ind,[1,0]], sample_coords_rgb[scale_ind,[3,2]] - sample_coords_rgb[scale_ind,[1,0]] - 1))
    ...
    def get_sample_location(self, sample_coord_rgb, sample_coord_d):
        """Get the location of the extracted sample."""

        sample_coord_rgb = sample_coord_rgb.float()
        sample_pos_rgb = 0.5*(sample_coord_rgb[:,:2] + sample_coord_rgb[:,2:] - 1)
        sample_scales_rgb = ((sample_coord_rgb[:,2:] - sample_coord_rgb[:,:2]) / self.img_sample_sz).prod(dim=1).sqrt()

        sample_coord_d = sample_coord_d.float()
        sample_pos_d = 0.5*(sample_coord_d[:,:2] + sample_coord_d[:,2:] - 1)
        sample_scales_d = ((sample_coord_d[:,2:] - sample_coord_d[:,:2]) / self.img_sample_sz).prod(dim=1).sqrt()

        return sample_pos_rgb, sample_pos_d, sample_scales_rgb, sample_scales_d
    ...
    def classify_target(self, sample_x_rgb: TensorList, sample_x_d: TensorList):
        """Classify target by applying the DiMP filter."""
        with torch.no_grad():
            # scores = self.net.classifier.classify(self.target_filter, sample_x)
            scores_rgb = self.net_rgb.classifier.classify(self.target_filter_rgb, sample_x_rgb)
            scores_d = self.net_d.classifier.classify(self.target_filter_d, sample_x_d)
        return scores_rgb, scores_d
    ...
    def extract_backbone_features(self, im: torch.Tensor, dp: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords_rgb = sample_patch_multiscale(im, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        dp_patches, patch_coords_d = sample_patch_multiscale(dp, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat_rgb = self.net_rgb.extract_backbone(im_patches)
            backbone_feat_d = self.net_d.extract_backbone(dp_patches)

        return backbone_feat_rgb, backbone_feat_d, patch_coords_rgb, patch_coords_d, im_patches, dp_patches

    def get_classification_features(self, backbone_feat_rgb, backbone_feat_d):
        with torch.no_grad():
            return self.net_rgb.extract_classification_feat(backbone_feat_rgb), self.net_d.extract_classification_feat(backbone_feat_d)

    def get_iou_backbone_features(self, backbone_feat_rgb, backbone_feat_d):
        for key in backbone_feat_rgb.keys():
            print('rgb: ', key)
        for key in backbone_feat_d.keys():
            print('d: ', key)
        return self.net_rgb.get_backbone_bbreg_feat(backbone_feat_rgb), self.net_d.get_backbone_bbreg_feat(backbone_feat_d) # Song : layer2 and layer3

    def get_iou_features(self, backbone_feat_rgb, backbone_feat_d):
        with torch.no_grad():
            for key in backbone_feat_d.keys():
                print('dd : ', key)
            f_rgb, f_d = self.get_iou_backbone_features(backbone_feat_rgb, backbone_feat_d)
            return self.net_rgb.bb_regressor.get_iou_feat(f_rgb), self.net_d.bb_regressor.get_iou_feat(f_d)

    def get_iou_modulation(self, iou_backbone_feat_rgb, iou_backbone_feat_d, target_boxes):
        with torch.no_grad():
            return self.net_rgb.bb_regressor.get_modulation(iou_backbone_feat_rgb, target_boxes), self.net_d.bb_regressor.get_modulation(iou_backbone_feat_d, target_boxes)
    ...
    def generate_init_samples(self, im: torch.Tensor, dp: torch.Tensor) -> TensorList:
        ...
        im_patches_rgb = sample_patch_transformed(im, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms)
        im_patches_d = sample_patch_transformed(dp, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms)

        # Extract initial backbone features
        with torch.no_grad():
            init_backbone_feat_rgb = self.net_rgb.extract_backbone(im_patches_rgb)
            init_backbone_feat_d = self.net_d.extract_backbone(im_patches_d)

        return init_backbone_feat_rgb, init_backbone_feat_d
    ...
    def update_memory(self, sample_x: TensorList, target_box, learning_rate = None):
        ...
        replace_ind = self.update_sample_weights(self.sample_weights_rgb, self.previous_replace_ind, self.num_stored_samples, self.num_init_samples, learning_rate)
        self.previous_replace_ind = replace_ind

        # Update sample and label memory
        for train_samp, x, ind in zip(self.training_samples_rgb, sample_x, replace_ind):
    ...
    def init_iou_net(self, backbone_feat_rgb, backbone_feat_d):
        # Setup IoU net and objective
        for p in self.net_rgb.bb_regressor.parameters():
            p.requires_grad = False

        for p in self.net_d.bb_regressor.parameters():
            p.requires_grad = False
        ...
        # Get iou features
        iou_backbone_feat_rgb, iou_backbone_feat_d = self.get_iou_backbone_features(backbone_feat_rgb, backbone_feat_d)

        # Remove other augmentations such as rotation
        iou_backbone_feat_rgb = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat_rgb])
        iou_backbone_feat_d = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat_d])

        # Get modulation vector
        self.iou_modulation_rgb, self.iou_modulation_d = self.get_iou_modulation(iou_backbone_feat_rgb, iou_backbone_feat_d, target_boxes)

        if torch.is_tensor(self.iou_modulation_rgb[0]):
            self.iou_modulation_rgb = TensorList([x.detach().mean(0) for x in self.iou_modulation_rgb])
            self.iou_modulation_d = TensorList([x.detach().mean(0) for x in self.iou_modulation_d])
    ...
    def _overwrite_classifier_params(self, feature_dim):
        # Overwrite some parameters in the classifier. (These are not generally changed)
        pred_module_rgb = getattr(self.net_rgb.classifier.filter_optimizer, 'score_predictor', self.net_rgb.classifier.filter_optimizer)
        pred_module_d = getattr(self.net_d.classifier.filter_optimizer, 'score_predictor', self.net_d.classifier.filter_optimizer)

        if self.params.get('label_threshold', None) is not None:
            self.net_rgb.classifier.filter_optimizer.label_threshold = self.params.label_threshold
            self.net_d.classifier.filter_optimizer.label_threshold = self.params.label_threshold
        if self.params.get('label_shrink', None) is not None:
            self.net_rgb.classifier.filter_optimizer.label_shrink = self.params.label_shrink
            self.net_d.classifier.filter_optimizer.label_shrink = self.params.label_shrink
        if self.params.get('softmax_reg', None) is not None:
            self.net_rgb.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
            self.net_d.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
        if self.params.get('filter_reg', None) is not None:
            pred_module_rgb.filter_reg[0] = self.params.filter_reg
            pred_module_rgb.min_filter_reg = self.params.filter_reg

            pred_module_d.filter_reg[0] = self.params.filter_reg
            pred_module_d.min_filter_reg = self.params.filter_reg
        if self.params.get('filter_init_zero', False):
            self.net_rgb.classifier.filter_initializer = FilterInitializerZero(self.net_rgb.classifier.filter_size, feature_dim)
            self.net_d.classifier.filter_initializer = FilterInitializerZero(self.net_d.classifier.filter_size, feature_dim)
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if num_iter > 0:
            ...
            samples = self.training_samples_rgb[0][:self.num_stored_samples[0],...]
            target_boxes = self.target_boxes[:self.num_stored_samples[0],:].clone()
            sample_weights = self.sample_weights_rgb[0][:self.num_stored_samples[0]]

            # Run the filter optimizer module
            with torch.no_grad():
                self.target_filter, _, losses = self.net_rgb.classifier.filter_optimizer(self.target_filter_rgb,
            ...
            if plot_loss:
                if isinstance(losses, dict):
                    losses = losses['train']
                self.losses_rgb = torch.cat((self.losses_rgb, torch.cat(losses)))
                if self.visdom is not None:
                    self.visdom.register((self.losses_rgb, torch.arange(self.losses_rgb.numel())), 'lineplot', 3, 'Training Loss' + self.id_str)
                elif self.params.debug >= 3:
                    plot_graph(self.losses_rgb, 10, title='Training Loss' + self.id_str)

    def refine_target_box(self, backbone_feat_rgb, backbone_feat_d, sample_pos, sample_scale, scale_ind, update_scale = True):
        """Run the ATOM IoUNet to refine the target bounding box."""

        if hasattr(self.net_rgb.bb_regressor, 'predict_bb'):
            return self.direct_box_regression(backbone_feat_rgb, backbone_feat_d, sample_pos, sample_scale, scale_ind, update_scale)

        # Initial box for refinement
        init_box = self.get_iounet_box(self.pos, self.target_sz, sample_pos, sample_scale)

        # Extract features from the relevant scale
        iou_features_rgb, iou_features_d = self.get_iou_features(backbone_feat_rgb, backbone_feat_d)
        iou_features_rgb = TensorList([x[scale_ind:scale_ind+1,...] for x in iou_features_rgb])
        iou_features_d = TensorList([x[scale_ind:scale_ind+1,...] for x in iou_features_d])
        ...
        # Optimize the boxes
        output_boxes, output_iou_rgb, output_iou_d = self.optimize_boxes(iou_features_rgb, iou_features_d, init_boxes)
        ...
        output_iou_rgb = output_iou_rgb[keep_ind]
        output_iou_d = output_iou_d[keep_ind]
        ...
        _, inds_rgb = torch.topk(output_iou_rgb, topk)
        _, inds_d = torch.topk(output_iou_d, topk)
        predicted_box_rgb = output_boxes[inds_rgb, :].mean(0)
        predicted_box_d = output_boxes[inds_d, :].mean(0)
        predicted_iou_rgb = output_iou_rgb.view(-1, 1)[inds_rgb, :].mean(0)
        predicted_iou_d = output_iou_d.view(-1, 1)[inds_d, :].mean(0)

        # Get new position and size
        new_pos_rgb = predicted_box_rgb[:2] + predicted_box_rgb[2:] / 2
        new_pos_d = predicted_box_d[:2] + predicted_box_d[2:] / 2

        new_pos_rgb = (new_pos_rgb.flip((0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos
        new_pos_d = (new_pos_d.flip((0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos

        new_target_sz_rgb = predicted_box_rgb[2:].flip((0,)) * sample_scale
        new_target_sz_d = predicted_box_d[2:].flip((0,)) * sample_scale

        new_scale_rgb = torch.sqrt(new_target_sz_rgb.prod() / self.base_target_sz.prod())
        new_scale_d = torch.sqrt(new_target_sz_d.prod() / self.base_target_sz.prod())

        self.pos_iounet = new_pos_rgb.clone()
        # self.pos_iounet = new_pos.clone()
        print('new_pos rgb: ', new_pos_rgb)
        print('new_pos d  : ', new_pos_d)

        if self.params.get('use_iounet_pos_for_learning', True):
            self.pos = new_pos_rgb.clone()
            # self.pos = new_pos_d.clone()

        self.target_sz = new_target_sz_rgb
        # self.target_sz = new_target_sz_d

        if update_scale:
            self.target_scale = new_scale_rgb
            # self.target_scale = new_scale_d
            print('new_scale_rgb : ', new_scale_rgb)
            print('new_scale_d   : ', new_scale_d)

        # self.visualize_iou_pred(iou_features, predicted_box)


    def optimize_boxes(self, iou_features_rgb, iou_features_d, init_boxes):
        box_refinement_space = self.params.get('box_refinement_space', 'default')
        if box_refinement_space == 'default':
            return self.optimize_boxes_default(iou_features_rgb, iou_features_d, init_boxes)
        if box_refinement_space == 'relative':
            return self.optimize_boxes_relative(iou_features_rgb, iou_features_d, init_boxes)
        raise ValueError('Unknown box_refinement_space {}'.format(box_refinement_space))


    def optimize_boxes_default(self, iou_features_rgb, iou_features_d, init_boxes):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            outputs_rgb = self.net_rgb.bb_regressor.predict_iou(self.iou_modulation_rgb, iou_features_rgb, bb_init)
            outputs_d = self.net_d.bb_regressor.predict_iou(self.iou_modulation_d, iou_features_d, bb_init)

            if isinstance(outputs_rgb, (list, tuple)):
                outputs_rgb = outputs_rgb[0]
                outputs_d = outputs_d[0]

            outputs_rgb.backward(gradient = torch.ones_like(outputs_rgb))
            outputs_d.backward(gradient = torch.ones_like(outputs_d))
        ...
        return output_boxes.view(-1,4).cpu(), outputs_rgb.detach().view(-1).cpu(), outputs_d.detach().view(-1).cpu()


    def optimize_boxes_relative(self, iou_features_rgb, iou_features_d, init_boxes):
    ...
    def direct_box_regression(self, backbone_feat_rgb, backbone_feat_d, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        # Optimize the boxes
        output_boxes_rgb = self.net_rgb.bb_regressor.predict_bb(self.iou_modulation_rgb, iou_features_rgb, init_boxes).view(-1,4).cpu()
        output_boxes_d = self.net_d.bb_regressor.predict_bb(self.iou_modulation_d, iou_features_d, init_boxes).view(-1,4).cpu()

        # Remove weird boxes
        output_boxes_rgb[:, 2:].clamp_(1)
        output_boxes_d[:, 2:].clamp_(1)

        predicted_box_rgb = output_boxes_rgb[0, :]
        predicted_box_d = output_boxes_d[0, :]

        # Get new position and size
        new_pos = predicted_box_rgb[:2] + predicted_box_rgb[2:] / 2
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz - 1) / 2) * sample_scale + sample_pos
        new_target_sz = predicted_box_rgb[2:].flip((0,)) * sample_scale
